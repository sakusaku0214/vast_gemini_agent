            ...
            try:
                ...
                final_inputs.append({
                    ...
                    "content": [{
                        "type": "text",
                        "text": (
                            "このターンの調査予算に到達しました。追加ツールは使えません。"
                            "これまでに取得済みの証拠だけを使い、"
                            "『ここまでに確認できたデータ』『現時点の判断』『まだ未確認の点』を"
                            "具体的な値や状態を含めて簡潔に整理してください。"
                            "そのうえで、次に深掘りするなら何を確認すべきかを1〜3点示し、"
                            "最後に必ず『さらに深掘りしますか？ 続ける場合は「続けて」と返信してください。』"
                            "と尋ねてください。断定できない点は不確実性を明示してください。"
                        ),
                    }],
                })
                response = self.client.interact(
